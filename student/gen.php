...
<body>
    ...
    if (isset($_SESSION['username']) && isset($_GET['cid'])) {
        ...
        $ch = "select * from certificate where cid = '$cid' and uid='$id'";
        $ch_res = $conn->query($ch);
        if($ch_res->num_rows > 0){
            echo '<script>alert("Certificate has already been downloaded.");</script>';
            // header("Location: classroom.php");
            // exit();
        }
        else{    
        // Insert record into certificate table
        $cert = "insert into certificate (cid, uid, cert_id) values ('$cid', '$id', '$gen_id')";
        $conn->query($cert);
        }
    ?>
        ...
        <div class="mt-8">
            ...
            <button id="download-btn" onclick="generateCertificate()" class="btn mx-4">Download Certificate (PDF)</button>
        ...
        <script>
            function generateCertificate() {
                var element = document.getElementById('certificate-template');
                ...
                localStorage.setItem(cert_id, true);
            }
        </script>
