...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['instruct'])) {

    $inst = $_SESSION['instruct'];
    $getName = "select instructor_name from instructors where instructor_id = '$inst'";
    $res = $conn->query($getName);
    $name = $res->fetch_assoc()['instructor_name'];
...
$course_level = $_POST['course_level'];
	...
	if (isset($_POST['action']) && $_POST['action'] == 'add_course'){
        ...
        VALUES ('$cid', '$course_name','$course_code', '$description', '" . json_encode(array(
            ...
            'tags4' => $tags4,
            'tags5' => $tags5
        )) . "', '$name', '$inst', '$instructor_img', '$price', '$duration', '$module', '$course_level')";
        ...
        if ($result === TRUE && $conn->affected_rows > 0) {
            ...
            echo json_encode(array('success' => false, 'message' => 'No records were updated'));
        }
